...
describe('AuthenticateUserUseCase', () => {
  ...
  const makeCreateUser = () => {
    ...
    const authenticateUserUseCase = new AuthenticateUserUseCase(refreshTokensRepository, userRepository, hashProvider, dateProvider)
  ...
  it('must not be able to authenticate a user with non-existent email or invalid password', async () => {
    ...
    await expect(authenticateUserUseCase.execute({ ...loginUserPayload, email: 'invalid@email.com' }))
      .rejects.toEqual(new AppError('User or password does not match'))

    await expect(authenticateUserUseCase.execute({ ...loginUserPayload, password: 'invalidPass123' }))
      .rejects.toEqual(new AppError('User or password does not match'))

    await expect(authenticateUserUseCase.execute({ ...loginUserPayload, password: 'invalidPass' }))
